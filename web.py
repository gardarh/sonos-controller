    ...
    return jsonify(
        {
            ...
            "endpoints": [
                ...
                {
                    "method": "GET",
                    "path": "/speakers/<speaker_name>",
                    "description": "Get speaker info",
                },
                {
                    "method": "POST",
                    "path": "/speakers/<speaker_name>/play",
                    "description": "Play on speaker",
                },
                {
                    "method": "POST",
                    "path": "/speakers/<speaker_name>/pause",
                    "description": "Pause speaker",
                },
                {
                    "method": "POST",
                    "path": "/speakers/<speaker_name>/set-source",
                    "description": 'Set speaker source (body: {"source": "line-in" | "music"})',
                },
                {
                    "method": "POST",
...
def set_source(speaker_name: str) -> Response | tuple[Response, int]:
    ...
    if not data or "source" not in data:
        return jsonify(
            {"status": "error", "message": "Missing 'source' in request body"}
        ), 400
    ...
    if success:
        return jsonify(
            {
                "status": "ok",
                "message": f"Set {speaker_name} source to {data['source']}",
            }
        )
...
def set_volume(speaker_name: str) -> Response | tuple[Response, int]:
    ...
    if not data or "volume" not in data:
        return jsonify(
            {"status": "error", "message": "Missing 'volume' in request body"}
        ), 400
    volume = data["volume"]
